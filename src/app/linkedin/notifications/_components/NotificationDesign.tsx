...
function NotificationDesign({
  ...
  return (
    <li key={notification.id} className={`rounded-xl transition-css ${!notification.readAt ? "bg-[#dfeaf6]" :"bg-white "}`}>
      <Link href={notification.route} className="flex items-start sm:p-4 p-2 gap-2">
        ...
        <div className="flex-1 space-y-1 ">
          <div className="flex items-center justify-between sm:flex-nowrap flex-wrap gap-x-5">
            ...
            <span className="lg:text-xs sm:text-[11px] text-[9px] text-gray-500">{formatted}</span>
          </div>
