...
function EditCommentBox({
  ...
  return (
    <div className="relative">
      ...
      <div
        className={`sm:p-3 p-1 rounded-xl box shadow z-10 bg-white flex-col absolute right-0 min-w-fit gap-2 mt-1 ${dropDownMenu === comment.id ? "flex" : "hidden"}`}
      >
        {comment.userId === user.id && (
          <>
            <button
              onClick={handleEditComment}
              className="flex items-center w-fit text-nowrap gap-2 sm:text-[15px] text-[13px] text-slate-600 cursor-pointer font-semibold hover:bg-gray-50 rounded hover:shadow py-2 px-3"
            >
            ...
            <button
              ...
              disabled={loading}
              className="flex disabled:cursor-wait w-fit text-nowrap sm:text-[15px] text-[13px] disabled:shadow disabled:bg-gray-200 disabled:text-gray-500 items-center gap-2 text-slate-600 cursor-pointer font-semibold hover:bg-gray-50 rounded hover:shadow py-2 px-3"
            >
              <i className="sm:text-xl text-[18px]">
                <MdDeleteSweep />
              </i>
